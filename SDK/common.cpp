...
std::variant<lua_State*, error_info> load_script(lua_State* L, const fs::path& script_path) noexcept {
...
std::optional<error_info> spawn_script(lua_State* L, const fs::path& script_path) noexcept {
    auto ret = load_script(L, script_path);
    if (error_info* err = std::get_if<error_info>(&ret)) {
        return err->propagate();
    }
    lua_State* co = std::get<lua_State*>(ret);
    int status = lua_resume(co, L, 0);
    if (status != LUA_OK) {
        return error_info(luaL_checkstring(co, -1));
    }
    return std::nullopt;
} 
