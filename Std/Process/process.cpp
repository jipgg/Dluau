...
using std::vector;
using std::string;
struct Resolved_procinfo {
    string exe;
    vector<string> args;
};

static auto resolve_procinfo(lua_State* L) -> variant<string, Resolved_procinfo> {
    ...
    if (top > 1) {
        ...
        return Resolved_procinfo{.exe = std::move(exe), .args = std::move(args)};
    } else {
...
static auto system(lua_State* L) -> int {
    try {
        ...
        if (auto* shell_cmd = std::get_if<string>(&variant)) {
            ...
            auto& info = std::get<Resolved_procinfo>(variant);
...
static auto child_ctor(lua_State* L) -> int {
    try {
        if (PidType::is(L, 1)) {
            ChildType::make(L, PidType::check(L, 1));
            return 1;
        }
        auto var = resolve_procinfo(L);
        if (auto* cmd = std::get_if<string>(&var)) {
            ChildType::make(L, bp::shell, *cmd);
            return 1;
        } else {
            auto& info = std::get<Resolved_procinfo>(var);
            ChildType::make(L, bp::child(bp::exe = info.exe, bp::args = info.args));
            return 1;
        }
    } catch(bp::process_error& e) {
        lu::error(L, e.what());
