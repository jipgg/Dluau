...
#include "fs.hpp"
using ReaderType = TaggedType<Reader>;
static const char* tname = "fs_Reader";
template<> const char* ReaderType::type_name(){return tname;}

int reader_line_iterator(lua_State* L) {
    auto& reader = ReaderType::check_udata(L, lua_upvalueindex(1));
...
Reader& new_reader(lua_State* L, Reader&& v) {
    ReaderType::init(L, ReaderType::init_info{
        .index = index,
        .namecall = namecall,
    });
    return ReaderType::new_udata(L, std::move(v));
}
Reader& check_reader(lua_State* L, int idx) {
    return ReaderType::check_udata(L, idx);
}
