...
static const char* tname = "fs_Dir";
template<> const char* DirType::type_name(){return tname;}
...
static const DirType::Registry namecall = {
...
static const DirType::Registry index = {
...
static int div(lua_State* L) {
    ...
    fs::path path;
    if (FileType::is_type(L, 2)) {
        path = check_dir(L, 2).path;
    } else if (FileType::is_type(L, 2)) {
...
Dir& new_dir(lua_State* L, const Dir& v) {
    if (not DirType::initialized(L)) {
        DirType::init(L, DirType::init_info{
            .index = index,
            .newindex = newindex,
            .namecall = namecall,
            .meta = meta,
    ...
    standardize(path);
    return DirType::new_udata(L, {.path = std::move(path)});
}
Dir& check_dir(lua_State* L, int idx) {
    return DirType::check_udata(L, idx);
}
