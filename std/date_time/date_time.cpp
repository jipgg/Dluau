...
static int from_date_time(lua_State* L) {
...
//TODO add intermediate types to avoid type overflow
...
static void register_precise_clock_variant(lua_State* L, const char* as) {
    const luaL_Reg lib[] = {
        {"now", [](lua_State* L) -> int {
            new_precise_time(L, precise_time_type(ch::duration_cast<ch::nanoseconds>(Clock::now().time_since_epoch())));
            return 1;
        }},
        {nullptr, nullptr}
    };
...
LUMINSTD_API inline int module_date_time(lua_State* L) {
    ...
    register_precise_clock_variant<ch::steady_clock>(L, "precise_clock");
